...
class GridView extends \himiklab\sortablegrid\SortableGridView
{
    ...
    //public $layout = '{pagerSelect}{summary}<br/>{pager}<br/>{clipboard}\n{items}<br/>{pager}';
    public $layout = '{search}{summary}<br/>{pager}<br/>{clipboard}\n{items}<br/>{pager}';
    ...
    {
		$return = '';
        $return .= '<div id="data-table_length" class="dataTables_length">';
        $value = null;
        if (isset(\Yii::$app->session[get_class($this->filterModel) . "PerPage"]))
        {
            $value = \Yii::$app->session[get_class($this->filterModel) . "PerPage"];
        }

        $select = \yii\helpers\Html::dropDownList("pager_pages", $value, array_combine(range(10, 500, 5), range(10, 500, 5)), array(
                    'class' => 'pager_select',
                    'model' => get_class($this->filterModel)
                ));

        $return .= "<label>Показывать на странице:&nbsp;&nbsp;{$select}</label>";
        $return .= '</div>';
		return $return;
    }
	
	public function renderSearch() {
        return '<div id="data-table_filter" class="dataTables_filter"><label>&nbsp;&nbsp;&nbsp;'.
            \yii::t('app','Search').
        ':<input type="search" class="" placeholder="" aria-controls="data-table" name="Search[phrase]"></label></div>';
	}
