...
$(function() {
	...
	calcSum.val( calcSumVal );
	calcDay.val( calcDayVal );



	$('.js-range-sum').ionRangeSlider({
		from: calcSumVal,
		min: 1000,
		max: 30000,
		step: 1000,
		postfix: " <span class='icon-rub'></span>",
		hide_min_max: true,
		grid: false,
		force_edges: true,
		grid: true,
		...
		onChange: function(data){
			...
			var count = data.slider.find('.irs-single').html();
				data.slider.find('.irs-slider').tooltipster('content', count).tooltipster('reposition');

			var calcSumValCurrent = data.from;
            calcSum.val( calcSumValCurrent );
            $.ajax({
                url: '/site/recountsum',
                data: 'sum='+calcSumValCurrent+'&days='+$('#calc-day').val()
            }).success(function(data){
                calcSumVal = data;
                if($('#calc-total').length > 0) {
                    $('#calc-total').html(calcSumVal.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '))
                }
            })
		}
	});
    ...
    if($('#calc-total').length > 0) {
        $('#calc-total').html($('#calc-sum').val().toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '))
    }
    ...
    $('.js-range-term').ionRangeSlider({
		...
		onChange: function(data) {
            ...
            var calcDayValCurrent = data.from;
            calcDay.val(calcDayValCurrent);
            calcDayVal = calcDayValCurrent;

            if ($('#datepicker').length > 0) {
                var now = new Date();
                now.setDate(now.getDate() + calcDayValCurrent);
                $('#datepicker').val(now.toLocaleString());
                $('#datepicker-day').val(dayOfWeek[now.getDay()]);
                //alert(now.toLocaleString());
            }
            $.ajax({
                url: '/site/recountsum',
                data: 'sum='+$('#calc-sum').val()+'&days='+$('#calc-day').val()
            }).success(function(data){
                calcSumVal = data;
                if($('#calc-total').length > 0) {
                    $('#calc-total').html(calcSumVal.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '))
        ...
        onUpdate: function(data) {
            ...
            data.slider.find('.irs-slider').tooltipster({
				animation: 'fade',
				position: 'bottom',
				maxWidth: 240,
				offsetX: 1,
			    ...
			    functionInit: function(){
			        return data.from + ' дней';
			    }
			}).tooltipster('show').tooltipster('reposition');
            /*var count = data.slider.find('.irs-single').html();
            data.slider.find('.irs-slider').tooltipster('content', count).tooltipster('reposition');*/
            //$('div.tooltipster-base.tooltipster-dark.tooltipster-fade.tooltipster-fade-show').css('left',$('span.irs-slider.single.tooltipstered').offset().left-30);
		}
	});

    currDateSlider = $(".js-range-term").data("ionRangeSlider");

    if ($('#datepicker').length > 0) {
        var now = new Date();
        now.setDate(now.getDate() + ($('#calc-day').val() * 1));
        $('#datepicker').val(now.toLocaleString());
    }

    $(document).on('change','#datepicker',function(){
        if($('#calc-day').length > 0) {

            var now = new Date();
            var diff = +parseLocalDate($('#datepicker').val()) - (+now);

            diff = Math.round(diff / (1000*60*60*24));

            $('#calc-day').val(diff + 1);

            currDateSlider.update({
			    from: diff ,
                min: 1,
                max: 30,
                step: 1
            ...
            currDateSlider.moveByKey(true);
             /*$('.irs-slider.single.tooltipstered').select();
            pressChar('d');
            pressChar('d');
            pressChar('d');*/

            //$(".js-range-term").data("ionRangeSlider").update({to: diff, from: diff});
        }
    })
